...
class BuildCommand extends Command
{
    ...
    ';
    protected $description = 'Generates a new build of TailwindCSS for your project.';
    ...
    public function handle()
    {
        ...
        if (! File::exists($binFile)) {
            $this->error('Could not find the TailwindCSS binary. Please, run the `tailwindcss:download` before trying to build or configure the path to the binary in your config/tailwindcss.php file.');
        ...
        $generatedFile = config('tailwindcss.build_destination_file_path');
        ...
        $process = new Process(array_filter([
            $binFile,
            '-i', config('tailwindcss.build_source_file_path'),
            '-o', $generatedFile,
        ...
        if (! $this->option('watch') && $this->option('digest')) {
            $this->info('Generating the versioned mix-manifest.json file...');
