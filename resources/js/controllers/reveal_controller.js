...
export default class RevealController extends Controller {
  ...
  async _init(event, shouldOpen) {
    ...
    if (shouldOpen) {
        this._focusOnFirstVisibleInput();
    }
  }

  _focusOnFirstVisibleInput() {
      const firstVisibleInput = Array.from(this.element
        .querySelector(this.selector)
        .querySelectorAll('input'))
        .filter(input => input.offsetParent !== null)

      if (firstVisibleInput.length === 0) return;

      firstVisibleInput[0].focus();
  }
