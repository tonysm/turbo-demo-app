<div class="bg-white p-4 space-x-4 flex">
    ...
    >
        ...
        <div class="block">
            ...
            <textarea class="form-textarea w-full" name="content" id="" rows="2" placeholder="What's happening?">{{ old('content', $tweet->content) }}</textarea>
            @error('content')
        ...
        <div class="sm:flex sm:justify-between sm:items-center space-y-4 sm:space-y-0">
            ...
            <div class="space-x-1">
                @if($tweet->exists)
                    <a x-data @click="$dispatch('close')" href="{{ route('tweets.show', $tweet) }}" class="px-4 py-2 text-gray-500 hover:bg-gray-100 font-semibold rounded-full text-sm">
                        Cancel
                    </a>
