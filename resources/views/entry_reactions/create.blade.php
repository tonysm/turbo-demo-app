        ...
        <div class="h-full mx-auto space-y-12 max-w-7xl sm:px-6 lg:px-8">
            <div class="h-full p-2 bg-white rounded-lg shadow md:p-8 lg:p-16">
                <turbo-frame id="@domid($entry, 'create_reaction')">
                    <form x-data @click.stop action="{{ route('entries.reactions.create', $entry) }}" method="GET">
                    ...
                    <div class="grid grid-cols-9 gap-2 mt-4">
                        @forelse ($emojis as $emoji)
                            <form action="{{ route('entries.reactions.store', $entry) }}" method="POST" class="block mx-auto">
                                @csrf
